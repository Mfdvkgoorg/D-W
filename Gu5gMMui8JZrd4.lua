...
-- ==========================
-- รีเทิร์นทันทีถ้ากำลังโหลดสคริปต์หลักอยู่ (กันวนซ้ำหลัง obf)
do
    local gv = (getgenv and getgenv()) or _G
    if type(gv) == "table" and rawget(gv, "__TB_SUPPRESS_UI") then
        return
    end
end
...
local function destroyAll()
    ...
    -- เคลียร์ธงชั่วคราว (อนุญาตให้เปิด UI ใหม่ได้รอบหน้า)
    pcall(function()
        local gv = (getgenv and getgenv()) or _G
        if type(gv) == "table" then gv.__TB_SUPPRESS_UI = nil end
    end)
end

-- โหลดสคริปต์หลักแบบกัน UI ถูกเรียกซ้ำระหว่างโหลด (กัน stack overflow หลัง obf)
local function loadMainScript()
    local gv = (getgenv and getgenv()) or _G
    if type(gv) == "table" then gv.__TB_SUPPRESS_UI = true end

    local ok, err = pcall(function()
        loadstring(game:HttpGet(MAIN_SCRIPT_URL))()
    end)

    -- ปลดธงเสมอ ไม่ว่าจะสำเร็จหรือพลาด เพื่อไม่ล็อกครั้งถัดไป
    if type(gv) == "table" then gv.__TB_SUPPRESS_UI = nil end
    return ok, err
end
