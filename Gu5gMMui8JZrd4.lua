-- กันโหลดซ้ำแบบตรวจ GUI จริง: ถ้ามี GUI ค้างอยู่ค่อย return
local GUI_NAME = "TB_GetKey_UI_Futuristic"  -- ใช้ชื่อเดียวกับด้านล่าง
local GUARD_KEY = "TB_UI_GETKEY_VER3"

if getgenv then
    local gv = getgenv()
    local already = rawget(gv, GUARD_KEY)
...
-- เปิด/ปิด debug log UI (ตั้งค่าได้ที่บนสุดของไฟล์)
