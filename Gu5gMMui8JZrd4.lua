-- ==========================
-- กัน UI ถูกโหลดซ้ำภายในช่วงสั้น ๆ แบบข้ามสคริปต์/ข้าม env (ทนต่อ obf)
do
    local Players = game:GetService("Players")
    local lp = Players.LocalPlayer
    local pg = lp and lp:FindFirstChildOfClass("PlayerGui")
    local cg = game:FindFirstChild("CoreGui")
    local parent = pg or cg or game
    local NAME  = "__TB_UI_MUTEX"
    local TTL   = 3  -- วินาทีที่ถือว่าเป็นการเรียกซ้ำ

    local now = tick()
    local m = parent and parent:FindFirstChild(NAME)
    if m then
        local last = tonumber(m:GetAttribute("t")) or 0
        if (now - last) < TTL then
            return  -- ภายใน TTL ถือว่าเป็นการเรียกซ้ำ → ตัดวนทันที
        end
        -- อัปเดตเวลาแล้วไปต่อ (เผื่อมีของเก่าค้าง)
        pcall(function() m:SetAttribute("t", now) end)
    else
        -- สร้างมิวเท็กซ์ใหม่
        m = Instance.new("Folder")
        m.Name = NAME
        m.Parent = parent
        m:SetAttribute("t", now)
    end
end
...
local function destroyAll()
    ...
    pcall(function() if getgenv then getgenv().TB_UI_GETKEY_VER3 = nil end end)

    -- ล้างมิวเท็กซ์ใน DataModel เพื่อให้เรียก UI ใหม่รอบหน้าได้
    pcall(function()
        local Players = game:GetService("Players")
        local lp = Players.LocalPlayer
        local pg = lp and lp:FindFirstChildOfClass("PlayerGui")
        local cg = game:FindFirstChild("CoreGui")
        local parent = (gui and gui.Parent) or pg or cg
        if parent then
            local m = parent:FindFirstChild("__TB_UI_MUTEX")
