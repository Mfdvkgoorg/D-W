-- ==========================
-- กัน UI ถูกโหลดซ้ำภายในช่วงสั้น ๆ (เช่นจาก main/obf เรียกวนกลับมา)
do
    local now = (task and task.wait and os and os.clock and os.clock()) or os.time()
    local ttl = 3  -- วินาทีที่ถือว่าเป็นการวนซ้ำ (ปรับได้ 2–5)
    local function hot()
        local gv = (getgenv and getgenv()) or _G
        if type(gv) ~= "table" then return false end
        local last = rawget(gv, "__TB_UI_RLOCK")
        if last and (now - last) < ttl then
            return true
        end
        gv.__TB_UI_RLOCK = now
        return false
    end
    if hot() then return end
end
...
local function destroyAll()
    ...
    pcall(function() if getgenv then getgenv().TB_UI_GETKEY_VER3 = nil end end)  -- ให้รันใหม่ได้

    -- เคลียร์รีเอ็นทรีล็อก เพื่อให้เรียก UI ใหม่รอบถัดไปได้
    pcall(function()
        local gv = (getgenv and getgenv()) or _G
        if type(gv) == "table" then
            gv.__TB_UI_RLOCK = nil
        end
    end)
end
